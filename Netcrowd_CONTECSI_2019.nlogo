...
;; -------- Versão 13b -------- ;;;
; RESOLVIDO: Os trabalhadores só podem executar as tarefas as quais foram designados. Eles  estão  execu-
;            tando outras pelo caminho e isso irá interfeir na experiência do artigo.

;; -------- Versão 14 --------- ;;
; RESOLVIDO: Preciso recalcular a reputação para que ela tenha um valor maior e mais significativo.
; RESOLVIDO: Tarefas mais próximas ganham ponto positivo. Tarefas mais distantes ganham  ponto  negativo.
; RESOLVIDO: Descobrir como melhorar o "maximize reputation".

;; -------- Versão 15 --------- ;;
; RESOLVIDO: Arrumar as versões com maximização corrigida para priorizar a distância em segundo lugar.
; RESOLVIDO: Fazer NOT AvoidTaskExecutionConcurrence depois de arrumar o código.

;; -------- Versão 16 --------- ;;
; RESOLVIDO: Arrumar ShowWorkerLinkedTask

;; -------- Versão 17 --------- ;;
; RESOLVIDO: Faltou implementar a situação que o trabalhador executa as tarefas as esmo, ou sejam  quando
;            SearchNearestTask = false.
; RESOLVIDO: Ajuste no cálculo da reputação para fazer com que tarefas mais próximas ganhem  valores  me-
;            lhores de reputação.
; RESOLVIDO: Workerskill aumenta de acordo com o Ln de taskDifficulty.
; RESOLVIDO: Tem um erro nessa rotina. Quando não tem tarefa por perto o taskDifficulty está dando  zero.
;            Cuidado com o if  if (any? tasks-on patch xcor ycor)

;; -------- Versão 18 --------- ;;
; RESOLVIDO: Código da procedure MoveWorker foi fatorado para ficar mais enxuto.
; RESOLVIDO: Colocado 3 formas de se incrementar o skill de um trabalhador: linear, ln e sigmoide.
; RESOLVIDO: Now we show why the simulation has ended.
; RESOLVIDO: Havia um erro na rotina de  execução de  tarefas.  Quando  não  havia  tarefa  por  perto  o
;            "ln taskDifficulty" estava dando zero. Isso foi resolvido ao verificar se a tarefa havia si-
;            do executada. O cálculo só podia ser feito se a tarefa foi executada.
; RESOLVIDO: Para facilitar a visualização da  execução  das  tarefas  eu  criei  uma  variável  booleana
;            "executed?".

;; -------- Versão 19 --------- ;;
; RESOLVIDO: Agora não se pode ter mais do que 1000 trabalhadores ou 1000 tarefas no ambiente de execução.
; RESOLVIDO: A escolha dos critérios de maximização foi melhorado.

;; -------- Versão 20 --------- ;;
; RESOLVIDO: Mudei o nome do habilitador de ticks para que ficasse com lógica positiva. Ao invés de
;            "DisableTickCount" ficou "EnableTickCount".
; RESOLVIDO: A escolha dos critérios de maximização foi melhorado.

;; -------- Versão 21 --------- ;;
; TODO: Repensar o cálculo de reputação, pois agora o trabalhador não tem mais a distância como fator ne-
;       gativo.
; TODO: Arrumar o lugar dos cálculos de reputação e tals.
;
;--------------------------------------------------------------------------------------------------------
